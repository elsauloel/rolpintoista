...
- 🔲 **P105. La zona de alerta del escenario 2 no coincide con lo que se ve dibujado (2026-09-22, encontrado al probar).** `percepcionEvaluarRuta` (cuando quien camina NO está en sigilo y se acerca a un rival oculto) chequea, en cada paso de la ruta, la zona **propia de quien camina**, orientada hacia la dirección en la que se está moviendo en ESE paso — no la zona real y fija del token oculto (la que dibuja 👓 Ver conos, según su rotación). Es a propósito: importa hacia dónde está prestando atención quien camina, no hacia dónde mira el oculto. Pero es fácil de confundir (el dueño lo probó esperando que entrar en la zona naranja dibujada alrededor del oculto alcanzara, y no pasó nada) y **no hay forma de previsualizar** la zona que realmente se va a usar, porque depende del camino exacto que se arrastre. **Por decidir:** ¿se deja así (más realista: importa hacia dónde mira quien camina) o se simplifica para que use la zona real y fija del oculto (menos realista, pero coincide con lo que 👓 dibuja y es más fácil de entender y de testear)? Si se simplifica, `percepcionEvaluarRuta` pasaría a usar `rivalesDe(t).map(zonasClaves)` igual que `sigiloEvaluarRuta`, en vez de recalcular una zona por paso.
