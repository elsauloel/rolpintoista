...
- **`duelo.js`** (`Duelo`, 2026-09-26, pedido del dueño) — **ataque paso a paso en vivo** entre atacante y defensor (etapa 1: contacto, 1 contra 1; diseño en [`../docs/ataque-paso-a-paso.md`](../docs/ataque-paso-a-paso.md)). Un duelo es un documento de `campanas/<id>/duelos` que los dos lados escuchan y escriben. `Duelo.elegirObjetivo({yo, ataque, suelto})` (lista de tokens del mapa; «Sin objetivo» deja el ataque suelto de siempre), `Duelo.abrir(id)`, `Duelo.escuchar({abrir})` (aviso «⚔ X te ataca — Abrir»; el GM ve también los duelos ajenos) y `Duelo.disponible()`. Cada página define `window.DUELO_HOOKS = {soy(lado), atacar(duelo), evadir(duelo)}` (ficha: el personaje; gm-tools: los creeps) y dispara `window.dispatchEvent(new CustomEvent('tirada-registrada', {detail: {origen, r}}))` desde `registrarTirada`: así el duelo recoge el PdG y la Evasión que ya se tiran de siempre. Sin hooks (el mapa) el GM tira «a mano» por cualquiera de los dos. Empate: gana el defensor (**PLACEHOLDER**, no está definido en el manual). El mapa solo escucha y abre la Botonera/Acciones del defensor con el mensaje `abrir-duelo`. Necesita las reglas nuevas de `duelos` (hay que pegarlas). Los duelos de más de un día los borra el GM al entrar.
