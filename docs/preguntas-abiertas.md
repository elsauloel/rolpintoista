...
- 🔶 **P115. Dos formas de mejorar el crítico (dicho por el dueño, 2026-09-25; en elaboración; parte de P113).** Para diseñar armas, ítems y skills de "crítico mejorado" hay dos palancas distintas: **(1) Bajar el "rango" del crítico** = el número contra el que se compara la diferencia PdG − Evasión: un arma Tipo 4 con crítico incrementado pasa a necesitar diferencia 3 en vez de 4; **el mínimo siempre es 2**. **(2) Bajar el umbral de los efectos del d20**: por ejemplo que el ×2 salga con 6+ en vez de 7+ (y análogamente los demás cortes). **Nombres propuestos (a elegir):** (1) **Crítico frecuente** (sale más seguido); (2) **Crítico feroz** (pega más fuerte). **A confirmar:** (a) si al bajar el rango (1) también cambia el nivel N = diferencia ÷ rango (dobles y triples críticos más fáciles), que es lo lógico; (b) si (2) baja todos los cortes juntos (7→6, 17→16, y el 20 ×4 = 19+?) o solo el primero; (c) tope de acumulación de bonos de cada tipo (el mínimo 2 ya limita al (1)); (d) cuál es el nombre definitivo, para usarlo igual en armas, ítems, skills y la Guía de diseño. **Skills existentes que ya lo tocan:** "+1 al crítico" (Lisiar, Tajear, Degollar, Headshot, Apuntar…) y "crítico mejorado" (Headshot) estaban sin mecánica; con esto pasan a ser Crítico frecuente / Crítico feroz.
