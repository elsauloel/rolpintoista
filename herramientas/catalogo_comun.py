...
STATS_OK = {'def','tipo1','tipo2','tipo3','tipo4','tipo5','mov','bonos','eva','ini','pdg','crit','critpot','pdgcontra','pdgopor',
...
def item_js(it, con_imagen=True):
    ...
    if (it.get('efectoNombre') or '').strip():
        ...
        if (it.get('efectoPreset') or '').strip():
            partes.append("efectoPreset:'%s'" % esc_js(it['efectoPreset']))
