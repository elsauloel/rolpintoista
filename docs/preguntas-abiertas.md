...
- 🔲 **P101. Pajaritos no está implementado para creeps (bug encontrado 2026-09-22).** En la ficha, Pajaritos (`mitadPdgEva`) parte al medio el PdG y la Evasión. En gm-tools, el preset existe (`ESTADOS_PRESET_GM`) y la descripción promete lo mismo, pero `creepStatValor`/`creepNitrosMax` nunca lo chequean — un creep con Pajaritos no pierde nada. Falta sumarle a gm-tools el mismo `mitadPdgEva` que ya tiene `lisiado`.
