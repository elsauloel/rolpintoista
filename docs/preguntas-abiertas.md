...
- ⬜ **P-Grupos-mapa. Los tokens de un mapa y los creeps del grupo de gm-tools tienen que ser lo mismo — 2026-09-26.** Pedido del dueño (viendo el mapa con muchos tokens y gm-tools con menos creeps en el grupo «Mapa 2»): *«los tokens y sus fichas deben estar representados por igual en mapa y grupo»*. Hoy el grupo se arma **a mano** en gm-tools y se vincula al mapa; los tokens pueden existir sin estar en el grupo (creeps «sin grupo» con token) y a la inversa. **Propuesta:** que la pestaña del mapa en gm-tools se **arme sola**: muestra siempre los creeps que tienen token en ese mapa (sin armado manual) y los tokens sin creep detrás (sin vincular) quedan marcados para vincularlos o borrarlos. Falta confirmar con el dueño (¿reemplaza a los grupos manuales o convive con ellos?).
